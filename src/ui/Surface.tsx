 ...
 *  - `paper`  — бумага этапа: жёсткая карточка длинного чтения (протокол,
 *               лабораторный журнал) — своя у каждого этапа синтеза
 *               (`--color-paper`/`--color-paper-ink`), но контраст внутри
 *               всегда честный, посчитан вручную (см. `tokens.css`, все
 *               пары ≥4.5:1). Форма — прямой прямоугольник с тонкой рамкой и
 *               офсетной тенью, как подшитый в дело лист, не воздушная
 *               карточка.
 *  - `ground` — прозрачная поверхность прямо на сцене: текст поверх
 *               `--color-scene`, без собственной подложки.
 *

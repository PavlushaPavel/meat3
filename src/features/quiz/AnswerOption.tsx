...
/**
 * Один вариант ответа допуска, этап 5 «Контроль» (docs/SPEC.md §3.4).
 * `--color-accent` здесь синий прибора, `--color-danger` — жёлтая опасная
 * разметка (`tokens.css`, `[data-stage='stage5']`) — но состояние НИКОГДА не
 * передаётся только цветом (docs/SPEC.md, требование доступности): у верного
 * варианта и у выбранного неверного есть собственный знак (галка / крест) и
 * собственная текстовая подпись, а не только заливка. Разбор показывается
 * после ЛЮБОГО ответа, включая верный, — без него тест превращается в
 * угадайку.
 *

 ...
 * тесты ниже намеренно проверяют состояние В СЕРЕДИНЕ потока, а не только в
 * его конце, поэтому `advanceTimersToNextTimer` (перепрыгивает к ближайшему
 * таймеру целиком) здесь не годится.
 */
...
describe('ChatScreen — очистка при размонтировании', () => {
  it('размонтирование посреди потока очищает все таймеры (свои и вложенных ScreenLine)', () => {

...
/**
 * Слот видео — предметный кадр в приборной эстетике текущего этапа
 * (docs/SPEC.md §1, §3.5).
 *
 * Прямой прямоугольник, жёсткая рамка, офсетная тень — тот же язык, что у
 * `Button`/`Surface`. Уголки кадра читаются как рамка видоискателя или
 * обрезка фотографии из дела, не декор ради декора.
 *
 ...
 *
 * Пустое состояние не мёртвое: медленная строка развёртки (`.video-empty-scan`,
 * `globals.css`) идёт сверху вниз через весь кадр — тот же приём, что уже
 * держат петли света сцены (`.stage-light`, скан/дыхание по этапам), здесь
 * применённый к самому слоту. Читается как «прибор ждёт материал, сканирует
 * в ожидании сигнала», а не как ошибка. Чистый CSS: `prefers-reduced-motion`
 * гасится общим барьером в `globals.css`, отдельной проверки в компоненте не
 * нужно (тот же принцип, что у `.stage-light`).
 */
    ...
    >
      {hasSrc ? (
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-8 text-center">
          {/* Прибор ждёт материал, не сломанный плеер: строка развёртки идёт
              через весь кадр, метка — тонкая, вместо шкалы воспроизведения. */}
          <span aria-hidden className="video-empty-scan" />

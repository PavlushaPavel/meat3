...
/** Подсказка о пропуске реела (экран 0) — появляется через несколько секунд
 *  после начала, второстепенным цветом, и исчезает вместе с пропуском или
 *  завершением реела. Не в SPEC.md §4 дословно: реел изначально пропускаем
 *  тапом, но без единого намёка об этом первый экран воронки терял людей
 *  молча (см. отчёт финальной доводки). */
export const PROLOGUE_SKIP_HINT = 'Тапни, чтобы пропустить';
